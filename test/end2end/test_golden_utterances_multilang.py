...
mechanical expansion of that locale's own ``fuster_quotes.intent`` /
``fuster_lifespan.intent`` (this one lives under an ``intents/``
subfolder in every locale) / ``who.intent`` padatious template:
``(a|b|c)`` word-choice groups (including a trailing ``(x)?``
shorthand) are resolved to one alternative, ``[optional]`` tokens are
kept or dropped. No translation, no drafted prose.
...
NEGATIVE_UTTERANCES = [
    ...
    ("where is the international space station", "en-US", "ovos-skill-iss-location.openvoiceos"),
]
